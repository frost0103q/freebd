...
<div class="container">	
	...
	<div class="padding-right">
			...
			<div class="company-info <?php echo ($logo_position == 'left') ? 'left-company-logo' : 'right-company-logo' ;?>" itemscope itemtype="http://data-vocabulary.org/Organization">
				...
				<div class="content">
					...
					<?php if ( get_option( 'job_manager_enable_types' ) ) { 
							...
							if ( $types && ! is_wp_error( $types ) ) : 
								foreach ( $types as $type ) { ?>
									<span><strong><?php echo $type->name; ?></strong></span>
							<?php }
							endif;?>
						<?php } ?>
						<br/>
						<?php if(class_exists('Astoundify_Job_Manager_Companies')) { echo workscout_get_company_link(the_company_name('','',false)); } ?>
						<?php the_company_name( '<span style="color: #000;font-size: 15px;font-weight: initial;">', '</span>' ); ?>
						</br>
						<?php if(class_exists('Astoundify_Job_Manager_Companies')) { echo "</a>"; } ?>
					...
					<?php if (in_array("job_title", $overview_elements)) : ?>
						<div>
						<span><strong style="color: #26ae61;"><?php the_title(); ?></strong></span>						
						</div>
						<?php endif;?>
		...
		<?php } ?>
				...
				<!-- modify by Jfrost -->
				<?php 
					$upload_url = get_post_meta( $post->ID, '_upload', true );
					if ($upload_url){ ?>
						<img class="" src="<?= $upload_url?>" style="border-radius : 5px; border: 1px solid lightgray;width: 97%;margin-bottom: 35px;">
					<?php }
				?>
	</div>
